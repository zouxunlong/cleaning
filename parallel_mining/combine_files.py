    ...
    for root, dirs, files in os.walk(rootdir):
        for file in files:
            if file.endswith('.en-ta') or file.endswith('.EN-TA'):
                inject_from_file(os.path.join(root, file), str(rootdir)+'.en-ta')
                file_combined+=1
            if file.endswith('.en-ms') or file.endswith('.EN-MS'):
                inject_from_file(os.path.join(root, file), str(rootdir)+'.en-ms')
                file_combined+=1
            if file.endswith('.en-zh') or file.endswith('.EN-ZH'):
                inject_from_file(os.path.join(root, file), str(rootdir)+'.en-zh')
                file_combined+=1

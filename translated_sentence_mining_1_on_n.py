...
embeddings_src = model.encode(
    sentences_src, show_progress_bar=True, convert_to_numpy=True)
...
embeddings_tgt_n_gram = model.encode(
    sentences_tgt_n_gram, show_progress_bar=True, convert_to_numpy=True)
...
with open(file_output, 'w', encoding='utf8') as fOut:
    for i in np.argsort(-scores):
        ...
        if src_ind not in seen_src and trg_ind not in seen_trg:
            ...
            fOut.write("{:.4f} | {} | {}\n".format(scores[i], sentences_src[src_ind].replace(
                "|", " "), sentences_tgt_n_gram[trg_ind].replace("|", " ")))
            sentences_written += 1
